...
export async function runSplit(input: SplitInput): Promise<SplitResult> {
  ...
  log?.(`parsed ${files.length} files, ${allHunks.length} hunks`)
  ...
  for (const layer of LAYERS) {
    ...
    if (hunkIds.size === 0) {
      log?.(`layer ${layer}: no hunks`)
      continue
    }
    log?.(`applying layer ${layer} (${hunkIds.size} hunks)`)
